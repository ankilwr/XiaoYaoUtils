...
/**
 * @param format: 例：%s/%s, 当前输入文字数量/最大的文字数量
 */
fun EditText.lengthLinkage(lengthShowView: TextView, format: String) {
    lengthShowView.text = String.format(format, text.length, this@lengthLinkage.maxLength())
    this.addTextChangedListener(object : TextWatcher {
        val tvText = WeakReference(lengthShowView)
        override fun beforeTextChanged(s: CharSequence, start: Int, count: Int, after: Int) {}
        override fun onTextChanged(s: CharSequence, start: Int, before: Int, count: Int) {}
        override fun afterTextChanged(s: Editable) {
            tvText.get()?.text = String.format(format, s.length, this@lengthLinkage.maxLength())
        }
    })
}
